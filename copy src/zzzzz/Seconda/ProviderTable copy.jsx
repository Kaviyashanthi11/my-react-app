...
function ProviderTable({
  ...
  useEffect(() => {
    if (showPagination) {
      ...
      // Display all rows when pagination is disabled
      setPaginatedRows(rows);
    }
  }, [page, rows, pageSize, showPagination]);
  ...
  const dataGridHeight = showPagination
    ? 410
    : Math.min(rows.length * 52, 410); // Adjust the height based on row count when pagination is off
  const bottomPadding = showPagination ? "24px" : "24px"; // Adjust bottom padding based on `showPagination`
  ...
  return (
    ...
    >
      <StyledDataGrid
        rows={paginatedRows} // Show all rows when pagination is off
        columns={columns
          .filter(
            ({ field }) =>
              !hiddenColumns.includes(field) &&
        ...
        autoHeight={true} // Automatically adjust height to fit all rows
      />
