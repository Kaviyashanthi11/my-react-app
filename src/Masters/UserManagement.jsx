...
useEffect(() => {
  if (selectedPractice && practiceOptions.length > 0) {
    fetch(`/React/web/index.php?r=api/super-admin-user-view&clientId=${selectedPractice}`)
      .then((response) => response.json())
      .then((result) => {
...
const handleSubmit = async (e) => {
    ...
    try {
        const response = isEditing
            ? await fetch(`/React/web/index.php?r=api/super-admin-user-save&clientId=${selectedPractice}&id=${formData.iUserId}`, {
                method: "PUT",
                headers: {
            ...
            : await fetch(`/React/web/index.php?r=api/super-admin-user-save&clientId=${selectedPractice}`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
...
// };
  ...
  return (
    <Layout> 
      ...
      {!isEditing && (
        <>
      <Typography style={{ marginRight: '1600px', whiteSpace: 'nowrap', color: '#29B6F6' ,fontSize:'large'}}>
        User Management
      </Typography>
      <br />
