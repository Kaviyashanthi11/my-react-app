...
const EligibilityPayerEnrollment = () => {   
   ...
   const handleGoClick = (e) => {
    ...
    const apiUrl = `/React/web/index.php?r=api/eligibility-enrollment-list&practiceId=${selectedPractice}&providerId=${selectedProviders}`;
    fetch(apiUrl)
        .then(response => response.json())
        .then(data => {
            ...
            setTimeout(() => {
                setOpen(false);
            }, 3000); // Hide loading backdrop after 3 seconds
        })
        .catch(error => {
            console.error('Error fetching data:', error);
            setOpen(false); // Hide loading backdrop on error
            alert('Error fetching data. Please try again.'); // Optional error handling
        });
};
    ...
    const handleSave = (row, rowIndex) => {
        ...
        if (enrollmentStatus !== undefined) {
            ...
            fetch('/React/web/index.php?r=api/eligibility-enrollment-save', {
                method: 'POST',
                headers: {
        ...
        <Layout>
          ...
          style={{ width: '100%',
            margin: '0 auto', boxSizing: 'border-box' ,marginRight:'700px'}}>
             <Grid container spacing={2} style={{ marginBottom: '22px' }}>
      ...
      <Grid item xs={12} sm={8}> {/* Ensure each select has its own Grid item */}
        ...
        />
                ...
                gap: '10px', 
                    ...
                    <Button   onClick={handleGoClick} style={{backgroundColor: '#00e5ff', color: 'black', }}>
                        Go
                    </Button>
    ...
    export default EligibilityPayerEnrollment;
